    ...
    var UGMBot = {
        ...
        eventDjadvance: function(obj) {
            ...
            var blacklistSkip = setTimeout(function() {
                ...
                for (var bl in UGMBot.room.blacklists) {
                    if (UGMBot.settings.blacklistEnabled) {
                        if (UGMBot.room.blacklists[bl].indexOf(mid) > -1) {
                            API.sendChat(subChat(UGMBot.chat.isblacklisted, {
                                blacklist: bl
                            }));
                            if (UGMBot.settings.smartSkip) {
                                return UGMBot.roomUtilities.smartSkip();
                            } else {
            ...
            var timeLimitSkip = setTimeout(function() {
                if (UGMBot.settings.timeGuard && newMedia.duration > UGMBot.settings.maximumSongLength * 60 && !UGMBot.room.roomevent) {
                    var name = obj.dj.username;
                    API.sendChat(subChat(UGMBot.chat.timelimit, {
                        name: name,
                        maxlength: UGMBot.settings.maximumSongLength
                    }));
                    if (UGMBot.settings.smartSkip) {
                        return UGMBot.roomUtilities.smartSkip();
                    } else {
                ...
                if (format == 1) {
                    $.getJSON('https://www.googleapis.com/youtube/v3/videos?id=' + cid + '&key=AIzaSyDcfWu9cGaDnTjPKhg_dy9mUh6H7i4ePZ0&part=snippet&callback=?', function(track) {
                        if (typeof(track.items[0]) === 'undefined') {
                            var name = obj.dj.username;
                            API.sendChat(subChat(UGMBot.chat.notavailable, {
                                name: name
                            }));
                            if (UGMBot.settings.smartSkip) {
                                return UGMBot.roomUtilities.smartSkip();
            ...
            if (UGMBot.settings.historySkip) {
                ...
                var historySkip = setTimeout(function() {
                    for (var i = 0; i < apihistory.length; i++) {
                        if (apihistory[i].media.cid === obj.media.cid) {
                            UGMBot.room.historyList[i].push(+new Date());
                            alreadyPlayed = true;
                            API.sendChat(subChat(UGMBot.chat.songknown, {
                                name: name
                            }));
                            if (UGMBot.settings.smartSkip) {
                                return UGMBot.roomUtilities.smartSkip();
                    ...
                    if (!alreadyPlayed) {
                        UGMBot.room.historyList.push([obj.media.cid, +new Date()]);
                    }
                }, 2000);
            }
            if (user.ownSong) {
                API.sendChat(subChat(UGMBot.chat.permissionownsong, {
                    name: user.username
                }));
                user.ownSong = false;
            }
            clearTimeout(UGMBot.room.autoskipTimer);
            if (UGMBot.settings.autoskip) {
                var remaining = obj.media.duration * 1000;
                var startcid = API.getMedia().cid;
                UGMBot.room.autoskipTimer = setTimeout(function() {
            ...
            */
            ...
            afkremovalCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        if (UNM.settings.afkRemoval) {
                            UNM.settings.afkRemoval = !UNM.settings.afkRemoval;
                            clearInterval(UNM.room.afkInterval);
                            API.sendChat(subChat(UNM.chat.toggleoff, {
                                name: chat.un,
                                'function': UNM.chat.afkremoval
                            }));
                        } else {
                            UNM.settings.afkRemoval = !UNM.settings.afkRemoval;
                            UNM.room.afkInterval = setInterval(function() {
                                UNM.roomUtilities.afkCheck()
                            }, 2 * 1000);
                            API.sendChat(subChat(UNM.chat.toggleon, {
                                name: chat.un,
                                'function': UNM.chat.afkremoval
                            }));
                        }
            ...
            afkresetCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        UNM.userUtilities.setLastActivity(user);
                        API.sendChat(subChat(UNM.chat.afkstatusreset, {
                            name: chat.un,
                            username: name
                        }));
                    }
            ...
            afktimeCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        var lastActive = UNM.userUtilities.getLastActivity(user);
                        var inactivity = Date.now() - lastActive;
                        var time = UNM.roomUtilities.msToStr(inactivity);

                        var launchT = UNM.room.roomstats.launchTime;
                        var durationOnline = Date.now() - launchT;

                        if (inactivity == durationOnline) {
                            API.sendChat(subChat(UNM.chat.inactivelonger, {
                                botname: UNM.settings.botName,
                                name: chat.un,
                                username: name
                            }));
                        } else {
                            API.sendChat(subChat(UNM.chat.inactivefor, {
                                name: chat.un,
                                username: name,
                                time: time
                            }));
            ...
            autodisableCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        if (UNM.settings.autodisable) {
                            UNM.settings.autodisable = !UNM.settings.autodisable;
                            return API.sendChat(subChat(UNM.chat.toggleoff, {
                                name: chat.un,
                                'function': UNM.chat.autodisable
                            }));
                        } else {
                            UNM.settings.autodisable = !UNM.settings.autodisable;
                            return API.sendChat(subChat(UNM.chat.toggleon, {
                                name: chat.un,
                                'function': UNM.chat.autodisable
                            }));
                        }
            ...
            autoskipCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        if (UNM.settings.autoskip) {
                            UNM.settings.autoskip = !UNM.settings.autoskip;
                            clearTimeout(UNM.room.autoskipTimer);
                            return API.sendChat(subChat(UNM.chat.toggleoff, {
                                name: chat.un,
                                'function': UNM.chat.autoskip
                            }));
                        } else {
                            UNM.settings.autoskip = !UNM.settings.autoskip;
                            return API.sendChat(subChat(UNM.chat.toggleon, {
            ...
            autowootCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        API.sendChat(UNM.chat.autowoot);
                    }
                }
            },
            ...
            baCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        API.sendChat(UNM.chat.brandambassador);
                    }
                }
            },
            ...
            ballCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        var randomBall = Math.floor(Math.random() * UNM.chat.balls.length);
                        var randomSentence = Math.floor(Math.random() * 1);
                        API.sendChat(subChat(UNM.chat.ball, {
                            name: chat.un,
                            botname: UNM.settings.botName,
                            question: argument,
                            response: UNM.chat.balls[randomBall]
                        }));
                    }
                }
            },
            ...
            banCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        var permFrom = UNM.userUtilities.getPermission(chat.uid);
                        var permUser = UNM.userUtilities.getPermission(user.id);
            ...
            blacklistCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        var msg = chat.message;
                        if (msg.length === cmd.length) return API.sendChat(subChat(UNM.chat.nolistspecified, {
                            name: chat.un
                        }));
                        var list = msg.substr(cmd.length + 1);
                        if (typeof UNM.room.blacklists[list] === 'undefined') return API.sendChat(subChat(UNM.chat.invalidlistspecified, {
                            name: chat.un
                        }));
                        else {
                            ...
                            UNM.room.newBlacklisted.push(track);
                            UNM.room.blacklists[list].push(media.format + ':' + media.cid);
                            API.sendChat(subChat(UNM.chat.newblacklisted, {
                                name: chat.un,
                                blacklist: list,
                                author: media.author,
                                title: media.title,
                            ...
                            if (UNM.settings.smartSkip && timeLeft > timeElapsed) {
                                UNM.roomUtilities.smartSkip();
                            } else {
                                API.moderateForceSkip();
                            }
                            if (typeof UNM.room.newBlacklistedSongFunction === 'function') {
                                UNM.room.newBlacklistedSongFunction(track);
                            }
                        }
                    }
                }
            ...
            blinfoCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        API.sendChat(subChat(UNM.chat.blinfo, {
                            name: name,
                            author: author,
                            title: title,
                            songid: songid
            ...
            bouncerPlusCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        var msg = chat.message;
                        if (UNM.settings.bouncerPlus) {
                            UNM.settings.bouncerPlus = false;
                            return API.sendChat(subChat(UNM.chat.toggleoff, {
                                name: chat.un,
                                'function': 'Bouncer+'
                            }));
                            ...
                            if (!UNM.settings.bouncerPlus) {
                                var id = chat.uid;
                                var perm = UNM.userUtilities.getPermission(id);
                                if (perm > API.ROLE.BOUNCER) {
                                    UNM.settings.bouncerPlus = true;
                                    return API.sendChat(subChat(UNM.chat.toggleon, {
                                        name: chat.un,
                                        'function': 'Bouncer+'
                                    }));
                                }
                            } else return API.sendChat(subChat(UNM.chat.bouncerplusrank, {
                                name: chat.un
                            }));
                        }
                    }
            ...
            botnameCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        var msg = chat.message;
                        if (msg.length <= cmd.length + 1) return API.sendChat(subChat(UNM.chat.currentbotname, {
                            botname: UNM.settings.botName
                        }));
                        var argument = msg.substring(cmd.length + 1);
                        if (argument) {
                            UNM.settings.botName = argument;
                            API.sendChat(subChat(UNM.chat.botnameset, {
                                botName: UNM.settings.botName
                            }));
                        }
                    }
            ...
            clearchatCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        return API.sendChat(subChat(UNM.chat.chatcleared, {
                            name: chat.un
                        }));
                    }
                }
            ...
            cmddeletionCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        if (UNM.settings.cmdDeletion) {
                            UNM.settings.cmdDeletion = !UNM.settings.cmdDeletion;
                            API.sendChat(subChat(UNM.chat.toggleoff, {
                                name: chat.un,
                                'function': UNM.chat.cmddeletion
                            }));
                        } else {
                            UNM.settings.cmdDeletion = !UNM.settings.cmdDeletion;
                            API.sendChat(subChat(UNM.chat.toggleon, {
            ...
            commandsCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        API.sendChat(subChat(UNM.chat.commandslink, {
                            botname: UNM.settings.botName,
                            link: UNM.cmdLink
                        }));
                    }
            ...
            cookieCommand: {
                ...
                getCookie: function(chat) {
                    var c = Math.floor(Math.random() * UNM.chat.cookies.length);
                    return UNM.chat.cookies[c];
                },
                functionality: function(chat, cmd) {
                    if (this.type === 'exact' && chat.message.length !== cmd.length) return void(0);
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        if (space === -1) {
                            API.sendChat(UNM.chat.eatcookie);
                            return false;
                        } else {
                            var name = msg.substring(space + 2);
                            var user = UNM.userUtilities.lookupUserName(name);
                            if (user === false || !user.inRoom) {
                                return API.sendChat(subChat(UNM.chat.nousercookie, {
                                    name: name
                                }));
                            } else if (user.username === chat.un) {
                                return API.sendChat(subChat(UNM.chat.selfcookie, {
                                    name: name
                                }));
                            } else {
                                return API.sendChat(subChat(UNM.chat.cookie, {
            ...
            cycleCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        UNM.roomUtilities.changeDJCycle();
                    }
                }
            },
            ...
            cycleguardCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        if (UNM.settings.cycleGuard) {
                            UNM.settings.cycleGuard = !UNM.settings.cycleGuard;
                            return API.sendChat(subChat(UNM.chat.toggleoff, {
                                name: chat.un,
                                'function': UNM.chat.cycleguard
                            }));
                        } else {
                            UNM.settings.cycleGuard = !UNM.settings.cycleGuard;
                            return API.sendChat(subChat(UNM.chat.toggleon, {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        if (!isNaN(cycleTime) && cycleTime !== '') {
                            UNM.settings.maximumCycletime = cycleTime;
                            return API.sendChat(subChat(UNM.chat.cycleguardtime, {
                                name: chat.un,
                                time: UNM.settings.maximumCycletime
                            }));
                        } else return API.sendChat(subChat(UNM.chat.invalidtime, {
                            name: chat.un
                        }));
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void(0);
                        ...
                        if (msg.length === cmd.length) name = chat.un;
                            ...
                            if (perm < API.ROLE.BOUNCER) return API.sendChat(subChat(UNM.chat.dclookuprank, {
                                name: chat.un
                            }));
                        }
                        var user = UNM.userUtilities.lookupUserName(name);
                        ...
                        var toChat = UNM.userUtilities.dclookup(user.id);
                        API.sendChat(toChat);
                    }
                }
            },
            ...
            deletechatCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!UNM.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length === cmd.length) return API.sendChat(subChat(UNM.chat.nouserspecified, {name: chat.un}));
                        var name = msg.substring(cmd.length + 2);
                        var user = UNM.userUtilities.lookupUserName(name);
                        if (typeof user === 'boolean') return API.sendChat(subChat(UNM.chat.invaliduserspecified, {name: chat.un}));
                        var chats = $('.from');
                        var message = $('.message');
                        var emote = $('.emote');
                        ...
                        API.sendChat(subChat(UNM.chat.deletechat, {name: chat.un, username: name}));
                    }
                }
            },
            */
